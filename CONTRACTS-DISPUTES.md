...
| D6 | §20 critic | `src/runtime/runtime.css` (L2) | `.pp-provenance` rendered `onPrimary` on `warning` — a pairing no solver post-condition covers — landing at **4.36:1** on the corpus. §14's rehearsal reported the provenance axis clean and the emitter then refused the same proof with fifty severity-1 `PROVENANCE_UNLABELED` findings. A rehearsal that clears what the emitter refuses is worse than no rehearsal | Fixed: the label wears `surfaceAlt`/`onSurfaceAlt`, the pair the solver guarantees and the validator checks; DECISIONS D24 |
| D7 | §20 critic | `src/core/text-metrics.js` (L1) | `resolveFace` returned `metricDelta: {cap:0,x:0,width:0}` for families with no published metrics — `metricsFor` had already fallen back to a category model, so the delta compared that model to itself and reported the substitution as metrically perfect exactly where we know least. Söhne, Bodoni Ultra Condensed and Comic Sans MS all came back exact | Fixed: `metricDelta: null` with `approximate: true`, which §4 permits; DECISIONS D25 |
| D8 | §20 critic | `src/runtime/keymap.js` (L2) | §11's return semantics were reachable only through the branch overlay, so a presenter leaving a branch had to open an overlay in front of the room | Fixed: `Backspace` → `returnOnce`, in the help overlay's `Branch` group; DECISIONS D26 |
| D9 | §20 critic | `scripts/build.mjs` (L1) | The runtime bundle shipped **158,804 bytes of source comments in 431,608** — 36.8% of every artifact — while §13's budgeter downscaled the prospect's own images, 3.8% of the same file | Fixed: `scripts/lib/strip-comments.mjs`, −37.8% on the script and −20.5% on the stylesheet, with the build refusing to emit if the stripped bundle's export surface moved by one name; DECISIONS D27 |
| D10 | §20 critic | `src/brand/color-css.js`, `src/brand/cluster.js` (L4) | `colorTokensIn` matched colour keywords **inside identifiers**, so `--nw-navy` yielded CSS `navy` and `url("gold-bar.png")` yielded `gold` — fabricated colours that arrived at full page-ground area weight and dominated the clustering. Separately, custom-property declarations were discarded entirely, so `background: var(--nw-navy)` contributed nothing | Fixed by L4: the keyword scan is bounded on both sides, strings and `url(...)` payloads are masked, and `var()` is fully resolved against a custom-property environment so a token contributes at the area it actually paints. L4-D18/D19/D20 |

| D11 | Corpus proof (integrator) | `src/scene/layouts/quote-card.js` (L8) | `quoteCard` renders `provenanceLabel` only on the branch where a quote block was found. A scene whose rendition section carries no quote falls through to the specimen quote or the headline and renders **no** label, while still declaring an illustrative rendition — so §18.1's law is broken by the layout and the emitter correctly refuses the whole artifact | Fixed by L8, with the other seven layouts audited for the same shape |
| D12 | Corpus proof (integrator) | `src/emit/scan.js` (L10) | The catch-all swept **every** attribute for absolute URLs at severity 1, so L8's `svg[aria-label]` accessible name for the system map refused the emit over a string nothing can request — and the same URL in a `<p>` passed, which penalised the accessible version of identical information | Fixed by L10 (E23): a closed named set of human-readable text attributes is prose with no finding at any severity. Every fetching attribute, `data-*`, `style`, `on*`, CSS, JS and `<meta content>` stay exactly as strict |
| D13 | Corpus `PLANTS` (integrator) | `src/validate/rules.js` (L11) | The **content** half of `DUPLICATE_SCENE` fingerprints over `beats[].reveals`, and reveals are derived from the scene id — so two scenes identical in layout, copy and renditions always fingerprint differently. It has never been able to fire for anything the id-collision half did not already catch | Routed to L11 |
| D14 | L10 | `src/validate/provenance.js` (L11) | A **second** reader for L7's promotion record, which `runPreflight` defaults to and which does not recognise records `promoteProvenance` actually writes. Preflight reported severity-1 `PROVENANCE_UNLABELED` against a correctly promoted rendition, so the studio disabled the emit button on an honest proof while `emit()` accepted it | Routed to L11: delete it and re-export L7's, as `lane-brand.js` and `lane-scene.js` already do for L4 and L8 |
| D15 | L10 | `src/validate/rules.js` (L11) | `ASSET_MISSING` requires a `kind: 'svg'` logo to be literal markup and refuses `data:image/svg+xml,…` at severity 1, though §4 documents `LogoAsset.data` as "inline SVG markup or data URI" for either kind — a severity-1 refusal of a proof the contract permits | Routed to L11 |
| D16 | L10's §14 gate | `src/scene/layouts/system-map.js` (L8) | `mapNodeMeta` clips a long source URL at severity 1 — 4.7% over at all three breakpoints with `text-overflow: clip`, so the truncation is silent. A real specimen URL hits it | Routed to L8 |
| D17 | L10 | `src/validate/lane-emit.js` (integrator) | Importing `scanForNetworkReferences`/`assertProvenance` through `emit/index.js` pulled `emit/emit.js` into the graph and closed a cycle the bundler refuses, which is why L10's §14 gate had to assemble `validate/rules.js` itself rather than calling `runPreflight` | Fixed: the bridge names the defining modules. L10-D8 |
